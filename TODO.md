...
- 🔲 **어종 도감** (기존 확장) — 30종 수집 현황, 최고 크기 기록, 구역별 분류(민물/강/바다/황금연못), 희귀도별 필터, 미수집 어종 실루엣 표시
- 🔲 **채집 도감** — 3종 허브(들풀/버섯/희귀허브) 총 채집 횟수·최다 채집 기록, 계절별 채집 통계
- 🔲 **광석 도감** — 4종 광석(철광석/구리광석/수정/금광석) 총 채굴 수, 층별 채굴 기록, 대박 발생 횟수
- 🔲 **요리 도감** — DISH_RECIPES 8종 + 기본 요리 완료 횟수, 첫 완성 날짜, 최고 판매가 기록
- 🔲 **제련/장신구 도감** — SMELT_RECIPES 4종·JEWELRY_RECIPES 4종 제작 횟수, 현재 장착 효과 한눈에 표시
- 🔲 **포션 도감** — POTION_RECIPES 7종 제조 횟수, 총 사용 횟수, 효과 설명
- 🔲 **작물 도감** — SEEDS 9종(기본 5 + 계절 4) 수확 횟수·최대 수확량, 계절 씨앗은 해당 계절 색상으로 표시
- 🔲 **도감 탭 통합 UI** — 기존 도감 패널에 탭 추가: 어종 / 채집 / 광석 / 요리 / 제작 / 포션 / 작물
- 🔲 **도감 완성 업적 추가** — 채집 완성("숲의 탐험가"), 광석 완성("채굴왕"), 요리 전 레시피 완성("마스터 셰프"), 작물 전 종류 수확("대지의 개척자"), 포션 전 종류 제조("연금술사")
- 🔲 **도감 완성률 HUD 표시** — 전체 도감 달성 % 스탯 패널 메인 탭에 표시
...
- 🔲 **낚시 미끼 DIY 제작** — 허브 + 광석 조합으로 커스텀 미끼 제작 (예: 버섯 3 + 구리광석 2 → 숲의미끼), gameData.js BAIT_RECIPES 추가
- 🔲 **납품 주문 시스템** — 매일 NPC 3종이 특정 요리/광석/허브 납품 요청, 완료 시 골드+친밀도 보너스 (일일 퀘스트와 별도)
...
- 🔲 **어종 생태계 시스템** — 특정 어종을 과도하게 잡으면 서버 전체 출현 확률 일시 감소 (-20%, 30분), 방류(인벤에서 물고기 방류 버튼)하면 회복 기여
- 🔲 **심해 원정대 (파티 전용)** — 파티원 3명 이상 심해 구역 진입 시 협동 보스 낚시 이벤트 발생, 초전설 어종(크라켄/해룡) 포획 가능
- 🔲 **농장 → NPC 납품 연동** — 작물 수확 시 NPC 납품 주문 자동 매칭, 잉여 작물 일괄 납품 버튼

### 8-5. 기술/UX 개선

- 🔲 **세이브 데이터 클라우드 백업** — 로그인 닉네임 기준 Firebase에 주기적 백업(5분마다), 세팅 패널에서 복구 버튼
- 🔲 **일일 미션 PWA 푸시 알림** — Service Worker 활용, 일일 퀘스트 초기화(자정) 시 선택적 푸시 알림
- 🔲 **도감 공유 기능** — 도감 현황 스크린샷 생성(canvas toDataURL), 공유 버튼 → 클립보드 복사 또는 다운로드
