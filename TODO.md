...
### 10-1. 채굴 접근 수정 (버그 수준)
- 🔲 `isInMineZone` 다중 존 지원 — 동쪽절벽·북쪽고원에 `ZONE_MINE_ZONES` 좌표 추가, `getActiveMineZone()` 함수로 존별 광산 영역 반환
- 🔲 `getActiveMineEntrance()` 동쪽절벽·북쪽고원 입구 타일 등록 (mapData.js)
- 🔲 GameCanvas 광산 입구 감지 로직 — `getActiveMineEntrance()` null 체크를 존별 값으로 교체
- 🔲 동쪽절벽 광산 UI 안내 메시지 — "절벽 광산 입구" HUD 표시

### 10-2. 월드 맵 오버뷰 UI
- 🔲 WorldMap.jsx 컴포넌트 — 5개 존 레이아웃 다이어그램 (마을 중심, 4방향 외곽 존)
- 🔲 현재 위치 하이라이트 + 존별 이름·아이콘·보너스 툴팁 표시
- 🔲 `M` 키 또는 모바일 버튼으로 열기, Sidebar 탐험 탭 내 통합 또는 별도 모달
- 🔲 각 존 보너스 요약 (ZONE_BONUSES 내용 표시)
...
### 10-4. 존 첫 방문 인트로 메시지
- 🔲 `DEFAULT_STATE.visitedZones: []` 추가, loadSave 마이그레이션
...
### 10-6. 외곽 존 여행 NPC
- 🔲 서쪽초원 — 행상인 NPC (초원 한복판): 허브/씨앗/낚시 소모품 할인 판매, 친밀도 보너스 없음
- 🔲 동쪽절벽 — 노련한 광부 NPC: 철수와 별개로 광석 감정(인벤 광석 1종 가격 알려주기) + 곡괭이 수리 기능
- 🔲 북쪽고원 — 산신령 NPC: 1일 1회 랜덤 버프 (낚시속도/채굴속도/판매가 중 1개 +10%, 30분)
- 🔲 남쪽심해 — 심해 탐험가 NPC: 심해 원정대 퀘스트 발급 + 크라켄/해룡 포획 시 특별 보상
